...
function build_prompt {
    ...
    if [[ $is_a_git_repo == true ]]; then
        ...
        if [[ $number_of_logs -eq 0 ]]; then
            ...
            local user_email=`git config --list | grep user.email | sed 's/user.email=//g'`
